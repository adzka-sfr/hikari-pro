...
// membuat sheet baru
$sheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Report U200'); //deklarasi nama sheet
...
for ($j = 1; $j <= $jumhar; $j++) {
    ...
    if ($listtude <= $tude) {
        ...
        $spreadsheet->setActiveSheetIndexByName('Report U200')->getStyle('A' . $bar)
    ...
    if ($hari == "Saturday" or $hari == "Sunday") {
        // Memeberi warna merah ketika sabtu dan minggu
        $spreadsheet->setActiveSheetIndexByName('Report U200')->getStyle('A' . $bar . ':G' . $bar)
            ->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID);
    ...
    // KOLOM ACTUAL (E)
    $sql3 = mysqli_query($connect_cm, "SELECT u200 from hasil where tanggal = '$har'");
...
$tgle = date('Y-m-d');
$writer->save('data_hasil/u200_' . $tgle . '.xlsx');
echo "<script>window.location = 'data_hasil/u200_" . $tgle . ".xlsx'</script>";
?>
...
$p_n = "export data u200"; // Nama Proses
$q = "read"; // Query
