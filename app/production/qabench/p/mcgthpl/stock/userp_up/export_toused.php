...
// membuat sheet baru
$sheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'User Package Used Today (UP)'); //deklarasi nama sheet
...
$sheet->setCellValue('B2', ': User Package Used Today (UP) - ' . date('y-m-d', strtotime($now)));
...
$sql = mysqli_query($connect_pro, "SELECT * FROM qa_userp WHERE c_used IS NOT NULL AND c_packed IS NOT NULL AND c_packed LIKE '$nowtoused%' AND c_location = 'packing up' ORDER BY c_packed DESC");
