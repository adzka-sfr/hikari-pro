...
// membuat sheet baru
$sheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Bench Used Today-GP'); //deklarasi nama sheet
...
$sheet->setCellValue('B2', ': Bench Used Today (GP) - ' . date('y-m-d', strtotime($now)));
...
$sql = mysqli_query($connect_pro, "SELECT * FROM qa_bench WHERE c_used IS NOT NULL AND c_packed IS NOT NULL AND c_packed LIKE '$nowtoused%' AND c_location = 'packing gp' ORDER BY c_packed DESC");
...
$tgle = date('Y-m-d');
$filename = 'Bench Used Today-GP.xlsx';
try {
