...
// membuat sheet baru
$sheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Stock Bench GP'); //deklarasi nama sheet
...
$sheet->setCellValue('B2', ': Stock Bench GP ');
...
$baris = 7;
$no = 1;
$sql = mysqli_query($connect_pro, "SELECT * FROM qa_bench WHERE c_used IS NOT NULL AND c_packed IS NULL AND c_location = 'packing gp'");
...
$tgle = date('Y-m-d');
$filename = 'Stock Bench GP - ' . $tgle . '.xlsx';
try {
