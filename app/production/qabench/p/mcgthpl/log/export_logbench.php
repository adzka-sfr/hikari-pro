...
$startdate = $_SESSION['logbench_export_start'];
$enddate = $_SESSION['logbench_export_end'];

// membuat spread sheet baru
$spreadsheet = new Spreadsheet();

// membuat sheet baru
$sheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Bench Stock Log'); //deklarasi nama sheet
...
$sheet->setCellValue('B2', ': Bench Stock Log');
...
$baris = 7;
$no = 1;
$sql = mysqli_query($connect_pro, "SELECT * FROM qa_log WHERE c_date >= '$startdate' AND c_date <= '$enddate' AND  c_gmcbench != '-' AND c_action = 'packing' OR c_date >= '$startdate' AND c_date <= '$enddate' AND c_gmcbench != '-' AND c_action = 'register' ORDER BY c_date DESC");
while ($data = mysqli_fetch_array($sql)) {
    ...
    $sheet->setCellValue('E' . $baris, $data['c_serialbench']);
    $sheet->setCellValue('F' . $baris, $data['c_namebench']);
