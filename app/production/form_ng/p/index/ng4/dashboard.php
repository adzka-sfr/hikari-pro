<div class="row">
    <div class="col-12">
        <h3>Outside Check 3 <span style="font-size: 10px;">(Checker now : <?= $_SESSION['nama'] ?>)</span> </h3>
...
if (isset($_POST['acard'])) {
    $_SESSION['cardnumber_outside3'] = $_POST['acard'];
}
?>
...
if (empty($_SESSION['cardnumber_outside3'])) {
    include('noform.php');
} else {
    // cek apakah slip terdaftar atau tidak
    $sql1 = mysqli_query($connect_pro, "SELECT id FROM formng_register where c_serialnumber = '$_SESSION[cardnumber_outside3]'");
...
?>
        ...
        // jika ada data
        $sql1 = mysqli_query($connect_pro, "SELECT * FROM formng_register WHERE c_serialnumber = '$_SESSION[cardnumber_outside3]'");
        $data1 = mysqli_fetch_array($sql1);

        if (empty($data1['c_finishoutcheck2'])) {
            unset($_SESSION['cardnumber_outside3']);
        ?>
            <script>
                $(document).ready(function() {
            ...
            <?php
        } elseif (empty($data1['c_finishcomplete3'])) {
            // dilakukan pengecekan apakah pengecek sama atau tidak
            $sql1 = mysqli_query($connect_pro, "SELECT * FROM formng_register WHERE c_serialnumber = '$_SESSION[cardnumber_outside3]'");
            $data1 = mysqli_fetch_array($sql1);

            if ($data1['c_complete1by'] == $_SESSION['nama']) {
                unset($_SESSION['cardnumber_outside3']);
            ?>
                <script>
                    $(document).ready(function() {
            ...
            <?php
            } elseif ($data1['c_complete2by'] == $_SESSION['nama']) {
                unset($_SESSION['cardnumber_outside3']);
            ?>
                <script>
                    $(document).ready(function() {
            ...
            <?php
                ...
                $sql1 = mysqli_query($connect_pro, "SELECT r.c_serialnumber, r.c_pianoname, c.c_category2 FROM formng_register r JOIN formng_category c ON r.c_gmc = c.c_gmc WHERE r.c_serialnumber = '$_SESSION[cardnumber_outside3]'");
                $data1 = mysqli_fetch_array($sql1);

                $_SESSION['serialnumber_outside3'] = $data1['c_serialnumber'];
                $_SESSION['pianoname_outside3'] = $data1['c_pianoname'];
                $_SESSION['complete_outside3'] = $data1['c_category2'];
                                ...
                                <script>
                                    $(document).ready(function() {
                                        Swal.fire({
                                            ...
                                            html: 'Silahkan menghubungi checker sebelumnya<br><b><?= $data3['c_checker3'] ?></b>',
                                            type: 'info',
                                            confirmButtonText: 'OK'
                                        }).then(function() {
                                            window.location = 'index.php';
