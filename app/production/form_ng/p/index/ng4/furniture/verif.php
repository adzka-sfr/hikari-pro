        ...
        <table class="table table-bordered">
            ...
            ?>
                <tr style="text-align: center; height: 10px;">
                    ...
                    <td><?= $section ?></td>
                    <td><?= $area ?></td>
                    <td style="text-align: left; width: 20%;"><?= $ng ?><br><b><?= $cab ?></b></td>
                </tr>
            <?php
            }
            ?>
...
if (isset($_POST['str'])) {
    ...
    $sql1 = mysqli_query($connect_pro, "SELECT id FROM formng_resulto1 WHERE c_serialnumber = '$serial_number' AND c_ng3 != ''");
    $data1 = mysqli_fetch_array($sql1);
    if (!empty(!empty($data1['id']))) {
        $sql1 = mysqli_query($connect_pro, "SELECT o.c_serialnumber, o.c_pianoname, o.c_checker3, o.c_section, o.c_areacode, o.c_ng3,o.c_cabinet, r.c_gmc  FROM formng_resulto1 o JOIN formng_register r ON o.c_serialnumber = r.c_serialnumber WHERE o.c_serialnumber = '$serial_number' AND c_ng3 != ''");

        while ($data1 = mysqli_fetch_array($sql1)) {
            $ppp1 = mysqli_query($connect_pro, "INSERT INTO formng_resultro SET c_serialnumber = '$data1[c_serialnumber]', c_pianoname = '$data1[c_pianoname]',c_gmc = '$data1[c_gmc]', c_inspectiondate = '$drn', c_checker = '$data1[c_checker3]', c_process = 'oc3', c_section = '$data1[c_section]', c_areacode = '$data1[c_areacode]', c_cabinet = '$data1[c_cabinet]', c_ng = '$data1[c_ng3]'");
        }
        if ($ppp1) {
    ?>
            <script>
        ...
        $data1 = mysqli_fetch_array($sql1);
        $ppp1 = mysqli_query($connect_pro, "INSERT INTO formng_resultro SET c_serialnumber = '$serial_number', c_pianoname = '$piano_name',c_gmc = '$data1[c_gmc]', c_inspectiondate = '$drn', c_checker = '$_SESSION[nama]', c_process = 'oc3'");
        if ($ppp1) {
        ?>
            <script>
                $(document).ready(function() {
