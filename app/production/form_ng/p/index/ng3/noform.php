    ...
    <div class="row">
        <div class="col-12">
            <div style="display: flex; flex-wrap: wrap; align-content: flex-start; gap: 1em;">
                ...
                $sql = mysqli_query($connect_pro, "SELECT DISTINCT rng.c_serialnumber FROM formng_resultong rng JOIN formng_repairdata rpd ON rng.c_serialnumber = rpd.c_serialnumber WHERE rpd.c_process = 'oc2' AND rng.c_checker = '$nama_checker' AND rng.c_repaired != '' AND rpd.c_endprocess IS NULL");
                while ($data = mysqli_fetch_array($sql)) {
                    $sql1 = mysqli_query($connect_pro, "SELECT COUNT(c_serialnumber) as isian FROM formng_resultong WHERE c_serialnumber = '$data[c_serialnumber]' AND c_process = 'oc2' AND c_repairdate IS NULL");
                    $data1 = mysqli_fetch_array($sql1);
                    if ($data1['isian'] == 0) {
                ?>
    ...
    <div class="row">
        <div class="col-12">
            <div style="display: flex; flex-wrap: wrap; align-content: flex-start; gap: 1em;">
                ...
                $sql = mysqli_query($connect_pro, "SELECT DISTINCT rng.c_serialnumber FROM formng_resultong rng JOIN formng_repairdata rpd ON rng.c_serialnumber = rpd.c_serialnumber WHERE rng.c_process = 'oc2' AND rng.c_checker = '$nama_checker' AND rng.c_repaired = '' AND rpd.c_endprocess IS NULL");
                while ($data = mysqli_fetch_array($sql)) {
                ?>
                    <!-- <button style="background-color: #ffa700; border-color: #ffa700;" class="btn btn-primary"><?= $data['c_serialnumber'] ?></button> -->
        ...
        <div class="col-12 mb-5">
            ...
            <table id="fin_ser" class="table table-bordered">
                ...
                <tbody>
                    ...
                    $no = 0;
                    $sql_finish = mysqli_query($connect_pro, "SELECT * FROM formng_register WHERE c_outcheck2by = '$_SESSION[nama]' AND c_finishoutcheck2 LIKE '$today_finish%' ORDER BY c_finishoutcheck2 ASC ");
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: center;"><?= $data_finish['c_finishoutcheck2'] ?></td>
                        </tr>
                    <?php
                    }
                    ?>
