...
if (isset($_POST['acard'])) {
    $_SESSION['cardnumber_outside2'] = $_POST['acard'];
}
?>
...
if (empty($_SESSION['cardnumber_outside2'])) {
    include('noform.php');
} else {
    // cek apakah slip terdaftar atau tidak
    $sql1 = mysqli_query($connect_pro, "SELECT id FROM formng_register where c_serialnumber = '$_SESSION[cardnumber_outside2]'");
...
?>
        ...
        // jika ada data
        $sql1 = mysqli_query($connect_pro, "SELECT * FROM formng_register WHERE c_serialnumber = '$_SESSION[cardnumber_outside2]'");
        $data1 = mysqli_fetch_array($sql1);

        if (empty($data1['c_finishoutcheck1'])) {
            unset($_SESSION['cardnumber_outside2']);
        ?>
            <script>
                $(document).ready(function() {
            ...
            <?php
        } elseif (empty($data1['c_finishcomplete2'])) {
