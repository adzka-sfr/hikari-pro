...
<script>
    $('#tabele').load('repairoutside/data.php').fadeIn("slow");
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status == 'OK') {
                    window.open("repairoutside/print.php", "_self");
                } else {
                    Swal.fire({
                        title: 'Gagal!',
                        icon: 'error',
    ...
    function btnPrint1(id, serialnumber) {
        ...
        $.ajax({
            ...
            success: function(response) {
                var response = JSON.parse(response);
                if (response.status == 'OK') {
                    window.open("repairoutside/print1.php", "_self");
                } else {
                    Swal.fire({
                        title: 'Gagal!',
                        icon: 'error',
