...
<div class="row">
    <div class="col-12">
        <button class="btn btn-secondary" id="inside" style="width: 100px; margin-bottom: 0px; border-top-left-radius: 10px; border-top-right-radius: 10px;">Inside</button>
        <button class="btn btn-secondary" id="outside" style="width: 100px; margin-bottom: 0px; border-top-left-radius: 10px; border-top-right-radius: 10px;">Outside</button>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            url: "management/c_ngtrend/inside.php",
            type: "POST",
            success: function(data) {
                $('#showpage').html(data);
                $('#inside').attr('disabled', true);
                $('#outside').attr('disabled', false);
            },
            error: function() {
                lostconnection()
            }
        ...
        $('#outside').click(function() {
            $.ajax({
                url: "management/c_ngtrend/outside.php",
                type: "POST",
                success: function(data) {
                    $('#showpage').html(data);
                    $('#inside').attr('disabled', false);
                    $('#outside').attr('disabled', true);
                },
                error: function() {
                    lostconnection()
                }
