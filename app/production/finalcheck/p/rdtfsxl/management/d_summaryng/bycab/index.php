...
<table class="table table-bordered dothemagictotable">
    ...
    <tbody>
        ...
        $q1 = mysqli_query($connect_pro, "SELECT c_code_cabinet, c_name FROM finalcheck_list_cabinet ORDER BY c_name ASC");
        while ($d1 = mysqli_fetch_array($q1)) {
            // get data last month
            $q2 = mysqli_query($connect_pro, "SELECT COUNT(c_serialnumber) as total FROM finalcheck_outside WHERE c_code_cabinet = '$d1[c_code_cabinet]' AND c_result_date LIKE '$bulan1%'");
            $d2 = mysqli_fetch_array($q2);

            // get data this month
            $q3 = mysqli_query($connect_pro, "SELECT COUNT(c_serialnumber) as total FROM finalcheck_outside WHERE c_code_cabinet = '$d1[c_code_cabinet]' AND c_result_date LIKE '$bulan2%'");
